...
@Composable
fun NoteAddScreen(noteViewModel: NoteViewModel) {
    val coroutineScope = rememberCoroutineScope()
    val noteUiState = noteViewModel.noteUiState.collectAsState()
    ...
    Column(
        ...
        // Row for picking date and time
        Row(
            horizontalArrangement = Arrangement.SpaceEvenly,
            modifier = Modifier.fillMaxWidth()
        ) {
            Column {
                AddDate(noteUiState.value.deadlineDate) { date ->
                    noteViewModel.setDate(date)
                }
            }

            Column {
                AddTime(noteUiState.value.deadlineTime) { time ->
            ...
            onClick = {
                coroutineScope.launch {
                    if (noteViewModel.isValid()) {
                        noteViewModel.upsertNoteWithoutId()
                        noteViewModel.resetValues()
                        isAdded.value = true
                    } else {
                        isError.value = true
        ...
        // Show the AlertDialog if there was an error (one of the field is not filled)
        if (isError.value) {
            AlertDialog(
                ...
                confirmButton = {
                    Button(
                        onClick = {
                            isError.value = false
                        }, // Reset the error state on button click
                        colors = ButtonDefaults.buttonColors(
                            backgroundColor = MaterialTheme.colors.surface,
                            contentColor = MaterialTheme.colors.onSurface
                        )
        ...
        // Show success message after adding the notes
        if (isAdded.value) {
            AlertDialog(
                onDismissRequest = { isAdded.value = false }, // Reset the error state on dismiss
                title = { Text(text = stringResource(R.string.success)) },
                text = { Text(text = stringResource(R.string.notes_has_been_added)) },
                confirmButton = {
                    Button(
                        onClick = {
                            isAdded.value = false
...
@Composable
fun AddDate(selectedDate: String, onDateSelected: (String) -> Unit) {
...
@Composable
fun AddTime(selectedTime: String, onTimeSelected: (String) -> Unit) {
